...
function ClientAuthorization(props) {
    ...
    const ClientBlurFocus=(event)=>{
         ...
         SetClientData({type:'initalEmail'});
         if((ErrorData.err==='true'))
         SetErrorData({type:'noerror'});
    }
...
props.onSubFunc( js.user);
    const userData={_id:js.user._id,
    Email:js.user.Email,
UserName:js.user.UserName};
    localStorage.setItem(props.localeData, JSON.stringify(userData));
    console.log(props.transferlink+js.user._id);
    if(props.localeData==='owner-data')
    Navigate(""+props.transferlink+js.user._id);
    }
  ...
  return (
    <form className='add-client1-form' id='Login-SingUp-Form' onSubmit={ClientDataSubmit}>
        ...
        <div className='add-client1-div'>
            ...
            <input className='add-client1-input' type='text' value={ ClientData.UserName} name="addUserName" onChange={ClientChange} onFocus={ClientBlurFocus} onBlur={ClientBlurFocus}></input>
        </div>
        <div className='add-client1-div'>
            <label className='add-client1-label'>Email:</label>
            <input className='add-client1-input' type='email' value={ ClientData.Email} name="addEmail" onChange={ClientChange} onFocus={ClientBlurFocus} onBlur={ClientBlurFocus}></input>
        </div>
        <div className='add-client1-div'>
            <label className='add-client1-label'>Password:</label>
            <input className='add-client1-input' type='password' value={ ClientData.Password} name="addPassword" onChange={ClientChange} onFocus={ClientBlurFocus} onBlur={ClientBlurFocus}></input>
        </div>
