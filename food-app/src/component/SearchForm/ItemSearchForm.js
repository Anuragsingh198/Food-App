...
function ItemSearchForm() {
    ...
    useEffect(() => {
        ...
        if (SearchList !== '') {
            ...
            for (let i = 0; i < hotelCtx.AllHotels.length; i++) {
                ...
                if (hotelCtx.AllHotels[i].Name.toLowerCase().includes(SearchList.toLowerCase())) {
                    console.log('cdssucsdbs')
                    ItemLists.push({ Name: hotelCtx.AllHotels[i].Name, Id: hotelCtx.AllHotels[i].Id, type: 'Restaurant' });
                }
            }
            if(hotelCtx.CategoryList!==null)
            for (let i = 0; i < hotelCtx.CategoryList.length; i++) {
                if (hotelCtx.CategoryList[i].toLowerCase().includes(SearchList.toLowerCase())) {
                    ItemLists.push({ Name: hotelCtx.CategoryList[i], type: 'Category' });
                }
            }
            if(hotelCtx.MenuList!==null)
            for (let i = 0; i < hotelCtx.MenuList.length; i++) {
                if (hotelCtx.MenuList[i].toLowerCase().includes(SearchList.toLowerCase())) {
    ...
    }, [SearchList,hotelCtx.MenuList,hotelCtx.CategoryList,hotelCtx.AllHotels])
    const SetBlurofDiv=()=>{
        setTimeout(()=>{
            SetDisplayVisibility(false);
        },300)
    ...
    return (
        <>
            <form className='Search-Item-Form' >
                <input className='Search-Item-Input ' type='text' placeholder='Search Reastaurant,Dishes,Cusinies' value={SearchList} onChange={(event) => {
                ...
                ></input>
                {
                    ...
                    <div className='Search-List-Display' >
                        {ItemList.map(item => <SearchItemsMapping key={item.Name + item.type} SearchFunc={SubmitSearchDataFunc} items={item} />)}
                    </div>}
            </form>
