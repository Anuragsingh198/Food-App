...
useEffect(()=>{
    async function getHotels(){
        ...
        if(Dishs) url=`http://localhost:4000/gethotels/${Params.locationid}?Dish=${Dish}`;
        if(Categorys) url=`http://localhost:4000/gethotels/${Params.locationid}?Category=${Category}`;
        ...
        if(js.status==='200')
        {
          ...
          {
            console.log(js)
          hotelCtx.AddHotels({Hotels:js.HotelData,
      ...
      {
        const datas=await fetch(`http://localhost:4000/gethotel/getdishes`,{
          method:'POST',
          body:JSON.stringify({
  ...
  return (
    <>
        <div className="Hotel-City-Location">Best Restaurants in {hotelCtx.Location} </div>
