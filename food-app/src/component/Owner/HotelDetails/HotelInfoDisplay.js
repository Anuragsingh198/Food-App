...
function HotelInfoDisplay(props) {
  return (
    ...
    <div className='Hotel-Data-Image'>
        <img src={props.HotelData.image} alt='Hotel Display Image'></img>
    </div>
    <div className='Hotel-Data-div'>
        <div className='Hotel-Data-Name'><div>{props.HotelData.name}</div>
        ...
        <div className='Hotel-Data-Category'>
          {props.HotelData.Categories.length===0&& <div>No category to Display</div>}
            {props.HotelData.Categories.map(item=><div key={item}>{item}</div>)}
        </div>
