...
const HotelDataReducerFunc=(state=intialState,action)=>{
    const Data={...state};
        ...
        Data.address=action.val;
        else if(action.type==='Rating')
        Data.Rating=action.val
        else if(action.type==='Count')
        Data.Count=action.val;
        else
        Data.image=action.val;
        return Data;
}
function ViewHotelDetails(props) {
    const [HotelData,SetHotelData]=useReducer(HotelDataReducerFunc,intialState);
    const [HotelMenu,SetHotelMenu]=useState([]);
    const params=useParams();
    const [hotelid,SetHotelId]=useState(null);
    if(hotelid!==params.hotelid)
    SetHotelId(params.hotelid);
    // console.log('getting data')
    useEffect(()=>{
        async function getDetails(){
            
            const Data= await fetch(`http://localhost:4000/${params.hotelid}/getdata`,{
                method:'GET'
            });
            ...
            if(js.status==='200')
            {
                ...
                SetHotelData({type:'AddAddress',val:js.hotel.City+' , '+js.hotel.Street});
                ...
                // console.log(typeof js.hotel.Menu+'   hii')
            }
        }
           getDetails();
    },[hotelid])
  return (
    ...
    <FoodDisplayCategories menu={HotelMenu} Name={HotelData.name}></FoodDisplayCategories>
    </div>
    </div>
    </>
  )
