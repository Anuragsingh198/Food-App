...
const UpdateValue = async (data) => {
  ...
  for (let i = 0; i < data.length; i++) {
    arr.push({ _id: data[i]._id, Location: data[i].Location, Hotels: data[i].Hotels[0] });
  }
  return arr;
}
const UpdatedHotelList = async (HotelData, FoodItem) => {
  ...
  for (let i = 0; i < HotelData.length; i++) {
    ...
    const foundItem = menu.Menu.reduce((acc, menuSchema) => {
      const item = menuSchema.items.find(item => { return item.Name === FoodItem });
      if (item) {
        return item;
      }
      return acc;
...
const UpdatedHotelCategoryList = async (HotelData, Category) => {
  ...
  for (let i = 0; i < HotelData.length; i++) {
    ...
    const foundItem = menu.Menu.find(item => { return item.Name === Category });
...
const DifferentDishes = async (HotelData) => {
  ...
  return { MenuList: NewMenuItem, Category: NewCategoryItem, HotelList: HotelList };
...
exports.getHotelForLocationName = (req, res, next) => {
  const name = re.body.LocationName;
...
exports.getLocationForCoordinates = async (req, res, next) => {
  ...
  if (!HotelDatas || !MenuData || !CategoryData||(!Dish&&!Category)) {
    const loca = await axios.get(`https://maps.googleapis.com/maps/api/geocode/json?address=${encodeURIComponent(address)}&key=${API_KEY}`)
  ...
  if (Dish) {
    UpdatedHotelList(HotelDatas, Dish).then(
      HotelData => {
    ...
    Hotels[i].Image=imageUrl
          ...
          res.json({
            status: '200', HotelData: Hotels, AllData: HotelDatas, MenuItem: MenuData,
    ...
    if(HotelDatas&&!HotelDatas[0].Rating)
    {
      const hotelIds = HotelDatas.map(item => item._id);
      Hotel.find({_id:{$in: hotelIds}})
      .then(result=>{
        res.json({
          status: '200', HotelData:result
        });
      })
    }
    else{
    res.json({
      status: '200', HotelData:HotelDatas
    });
  }
  }
...
exports.getDishes = async (req, res, next) => {
  ...
  if (Data) {
    console.log(Data)
    res.json({
      status: "200", AllData: Data.HotelList, MenuItem: Data.MenuList,
